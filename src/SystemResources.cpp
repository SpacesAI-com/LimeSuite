...
std::string lime::getLimeSuiteRoot(void)
{
    ...
    return "C:/Program Files (x86)/LimeSuite";
}
...
std::string lime::locateImageResource(const std::string &name)
{
    ...
    {
        const std::string fullPath(searchPath + "/23.11/" + name);
...
{
    const std::string destDir(lime::getAppDataDirectory() + "/images/23.11");
    const std::string destFile(destDir + "/" + name);
    const std::string sourceUrl("https://downloads.myriadrf.org/project/limesuite/23.11/" + name);
